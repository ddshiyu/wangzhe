module.exports = app => {
  ...
  router.get('/heroes/list', async (req, res) => {
    ...
    const newsData = await category.aggregate([
      ...
      {
        $lookup: {
          // heros 是模型小写的复数, 模型都用单数,,但是在模型中定义却没有用
          from: 'heros',
          localField: '_id',
          foreignField: 'categories',
          as: 'heroesList'
    ...
    newsData.unshift({
      ...
      newsList: await hero.find().where({
        categories: { $in: subCats }
      }).populate('categories').limit()lean()
    })
